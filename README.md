...
The county boundaries are based on county shapefiles from the U.S. Census 2010 [Cartographic Boundary shapefiles page](https://www.census.gov/geo/mapsdata/data/tiger-cart-boundary.html).

### Pulling hourly data

You can also use functions in the `countyweather` package to pull hourly weather data by county from NOAA's Integrated Surface Data (ISD) weather dataset. In this case, NOAA's web services will not identify weather stations by FIPS, so instead this function will pull all weather stations within a certain radius of the county's population mean center to represent weather within that county.

An estimated radius is calculated for each county using 2010 U.S. Census Land Area data - each county is assumed to be roughly ciruclar. The calculated radius (in km; e.g., `andrew_hourly$radius`), as well as the longitude and latitude of the geographic center for each county (e.g., `andrew_hourly$lat_center`, `andrew_hourly$lon_center`) are included as elements in the list returned from `hourly_fips`.
...
The `write_daily_timeseries` function saves each county's time series as a separate .rds file in a `data` subdirectory within the directory specified with the `out_directory` argument. The output directory will also include subdirectories with weather station metadata (`metadata`) and with png station maps (`maps`). At this stage, if you were to include a county in the `fips` argument without available data, a file would not be created for that county.

The function `plot_daily_timeseries` creates and saves plots for each of the .rds files in the `data` subdirectory.
