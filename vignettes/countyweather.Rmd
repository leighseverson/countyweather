...
You will also need an API key from NOAA to be able to access the weather data. You can get one by requesting that NOAA email you an API key from here: \url{http://www.ncdc.noaa.gov/cdo-web/token}. Next, open a new text file, and type the following (make sure to add a second blank line):
```{r}
noaakey == [your NOAA API key]

```

Save this text file as `.Renviron` in your home directory. Once you restart R, you can pull your key using `Sys.getenv("noaakey")`. Before using functions that require the API key, run: 

```{r}
options("noaakey" = Sys.getenv("noaakey"))
```

(Alternatively, you can run `options(noaakey = "your NOAA API key"`). 
