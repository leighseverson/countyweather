...
It might be interesting here to compare this plot with the station map, this time with station labels included (`station_label = TRUE` in our `daily_fips` call):  
...
There are other columns available in addition to these weather variables such as quality codes. For example, `wind_direction_quality` gives quality code information for the `wind_direction` variable. Each of the main weather variables has a corresponding quality code that can be pulled by adding `_quality` to the end of the variable name. 
...
This error message will come up after running `write_daily_timeseries` or `write_hourly_timeseries` if the function is unable to pull data for a particular fips code in your `fips` vector: 
