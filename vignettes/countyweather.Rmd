...
```{r warning = FALSE, message = FALSE, echo = 1, eval = FALSE}
...
There are a few functions that allow the user to write out daily or hourly timeseries datasets as `.rds` or `.csv` files to a specified local directory, as well as plots of this data. [Let's talk today-- if we only allow `.rds` output, then we could save a list object instead of a dataframe, so we could include station meta-data for each county.] For daily weather data, see the functions `daily_timeseries` and `plot_daily_timeseries`. For hourly, see `hourly_timeseries` and `hourly_plots`. 
...
`daily_timeseries()` saves each county's timeseries as a separate .rds file in the directory specified in the `out_directory` option. The function `plot_daily_timeseries` creates and saves plots for each of these .rds files [if we keep the `.csv` output option, does this also work if you've output files as `.csv`?]. 
...
[Maybe we want to show some code for replacing all observations with quality codes of 2, 3, 6, or 7 with `NA`s?]
